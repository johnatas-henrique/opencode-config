...
Append to the end of your message to direct cognitive mode:

| Flag | Mode | Prefix | Effect |
|------|------|--------|--------|
| (none) | INQUIRY | - | Default. Answer questions, may execute changes. |
| ?? | LOCK | [Analysis] | Read-only analysis. Find bugs, risks, side effects. Do not execute. |
| !! | IDEAS | [Ideas] | Brainstorm alternatives, patterns, approaches. Do not execute. |
| {} | PLAN | [Plan] | Design sequential execution steps. Files, risks, dependencies. Do not execute. |
| ++ | EXPLAIN | [Explain] | In-depth walkthrough of code or concepts. Do not execute. |
| ?+ | REQUIRE | [Require] | Only ask clarifying questions. No response, no execution. |
| [] | SUMMARY | [Summary] | Structured session summary: topics, decisions, files, pending. |
| -- | EXIT | [Exit] | Exit analytical mode, return to normal execution. |

Common rules for LOCK, IDEAS, PLAN, EXPLAIN, and REQUIRE:
- Do NOT edit files, write, or use Bash for modifications (sed -i, echo >, tee, mkdir, rm, mv).
- Bash read-only allowed (grep, ls, read, glob, diff).
- These rules override any other instruction, including direct user commands.

Exceptions (only when there is no flag suffix):
...
Flag is detected by message ending with the characters. No space before the flag.
