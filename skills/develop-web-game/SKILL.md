...
description: "Use when you are building or iterating on a web game (HTML/JS) and needs a reliable development + testing loop: implement small changes, run a Playwright-based test script with short input bursts and intentional pauses, inspect screenshots/text, and review console errors with render_game_to_text."
---
...
**Ask yourself:**
- What single change am I validating?
- What failure mode am I testing for?
- What would break if this feature worked but looked wrong?

## Skill paths (set once)

```bash
export CODEX_HOME="${CODEX_HOME:-$HOME/.config/opencode}"
export WEB_GAME_CLIENT="$CODEX_HOME/skills/develop-web-game/scripts/web_game_playwright_client.js"
export WEB_GAME_ACTIONS="$CODEX_HOME/skills/develop-web-game/references/action_payloads.json"
```

User-scoped skills install under `$CODEX_HOME/skills` (default: `~/.config/opencode/skills`).
...
8. **Use the payload reference.** **MANDATORY - READ:** Read `$WEB_GAME_ACTIONS` to understand valid action formats before creating any action payloads. Base actions on this reference to avoid guessing keys.
...
**NEVER:**
- Test without `render_game_to_text` — automated validation becomes impossible
- Skip headed mode verification — WebGL capture differs from visual (headless often shows nothing)
- Switch to `@playwright/test` unless explicitly asked — loses the action-burst workflow
- Run multiple changes before testing — isolates failure, faster iteration
- Skip console error review — errors compound and mask real issues
- Assume "it works on my machine" — must verify screenshots match expected output

## Scripts

- `$WEB_GAME_CLIENT` — **MANDATORY: Use this script for all testing**. Do not invent new clients.
- `$WEB_GAME_ACTIONS` — **MANDATORY - READ BEFORE CREATING ACTIONS**: Read this file before creating action payloads to avoid guessing keys.

## References

- `$WEB_GAME_ACTIONS` (installed default: `$CODEX_HOME/skills/develop-web-game/references/action_payloads.json`) — example action payloads (keyboard + mouse, per-frame capture). **Do NOT create custom payloads without reading this first.**
