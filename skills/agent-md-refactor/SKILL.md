...
- Language-specific conventions → `typescript.md`, `python-style.md`
- Testing guidelines → `testing.md`
- Code style details → `code-style.md`
- Framework patterns → `react-patterns.md`, etc.
- Documentation standards → `docs-style.md`
...
| Avoid | Why | Instead |
|-------|-----|---------|
| Keeping everything in root | Obscures critical overrides; Agent ignores walls of text | Split into linked files by topic |
| Too many categories (>10) | Fragmentation causes Agent to miss relevant instructions | Consolidate related topics (3-8 files ideal) |
| Vague instructions | Wastes tokens; Agent can't act on "be careful" | Be specific or delete |
| Duplicating defaults | Agent already knows TS/JS standards | Only override when project has non-standard rules |
| Deep nesting (H2 → H3 → H4) | Obscures hierarchy; Agent stops reading | Flat H2 structure with links to detailed files |
