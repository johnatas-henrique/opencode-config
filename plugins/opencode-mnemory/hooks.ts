  ...
  const eventHandler = async (input: { event: unknown }) => {
    ...
    if (event.type === "session.idle") {
      ...
      try {
        ...
        // Use "id" as the SDK path key (confirmed working across SDK versions).
        // "sessionID" was removed because the OpenCode server fails to interpolate
        // the {id} template when the path key doesn't match, causing a TUI error
        // on every call that gets logged before the catch block can suppress it.
    ...
    if (event.type === "session.compacted") {
      ...
      const sessionId = props?.sessionID;
      // Reject un-interpolated template (%7Bid%7D) — same fix as session.idle.
      if (!sessionId || sessionId.startsWith('ses_') === false) return;
